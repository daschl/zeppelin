...
/**
 * This {@link Interpreter} uses Couchbase Server and Analytics.
 *
 * @author Michael Nitschinger
 * @author Laurent Doguin
 */
public class CouchbaseAnalyticsInterpreter extends Interpreter {

  private Logger logger = LoggerFactory.getLogger(CouchbaseAnalyticsInterpreter.class);

  private volatile Cluster cluster;
  private volatile Bucket bucket;

  public CouchbaseAnalyticsInterpreter(final Properties property) {
    super(property);
  }
  ...
  public InterpreterResult interpret(String cmd, InterpreterContext interpreterContext) {
    logger.info("Running Analytics command '" + cmd + "'");

    try {
      AnalyticsQuery analyticsQuery = AnalyticsQuery.simple(cmd);
      ...
      Iterator<AnalyticsQueryRow> iter = result.rows();
      ...
      //
      while (iter.hasNext()) {
        AnalyticsQueryRow row = iter.next();
